...
CREATE OR REPLACE PROCEDURE decompress_dimension_slice(IN dimension_slice_row _timescaledb_catalog.dimension_slice,
                                                       INOUT chunks_decompressed bool)
AS
$$
DECLARE
...
CREATE OR REPLACE FUNCTION move_compression_job(IN hypertable_id int, IN schema_name name, IN table_name name,
                                                IN new_time timestamptz, OUT old_time timestamptz)
AS
$$
DECLARE
...
BEGIN
    ...
    IF version = 1 THEN
        ...
        FROM _timescaledb_config.bgw_policy_compress_chunks b
        WHERE b.hypertable_id = move_compression_job.hypertable_id;
    ELSE
        SELECT s.job_id
        INTO compression_job_id
        ...
        WHERE j.proc_name = 'policy_compression'
          AND s.hypertable_schema = schema_name
          AND s.hypertable_name = table_name;
    END IF;
...
DECLARE
    ...
    -- dest_tmp   text := 'events_raw (time, log_entry_hash,log_entry)'::text;
    dest_tmp                 text := FORMAT('%1$s (%2$s)', dest, cols);
    dest_nspname             name;
    dest_relname             name;
...
BEGIN
    ...
    IF cols = '*' THEN
        ...
        GROUP BY table_name;
        dest_tmp = FORMAT('%1$s (%2$s)', dest, cols);
    END IF;
    ...
    EXECUTE FORMAT('SELECT (%3$s)::%1$s FROM %2$s AS h LIMIT 1', source, dest, cols);
    ...
    WHERE table_name = dest_relname
      AND schema_name = dest_nspname;
    ...
    SELECT move_compression_job(hypertable_row.id, hypertable_row.schema_name, hypertable_row.table_name,
    ...
    ELSEIF UPPER(on_conflict_action) = 'UPDATE' THEN
        SELECT 'ON CONFLICT DO UPDATE SET ' ||
               STRING_AGG(FORMAT('%1$I = EXCLUDED.%1$I', on_conflict_update_column), ', ')
        FROM UNNEST(on_conflict_update_columns) AS on_conflict_update_column
        INTO on_conflict_clause;
    END IF;
    ...
    FOR dimension_slice_row IN
        ...
        WHERE dimension_id = dimension_row.id
          ...
          AND max_time_internal >= ds.range_start
        ...
        LOOP
            ...
            r_end_prev = COALESCE(r_end,
                                  _timescaledb_internal.time_literal_sql(min_time_internal, dimension_row.column_type));
    ...
    r_start = COALESCE(r_end, _timescaledb_internal.time_literal_sql(min_time_internal,
                                                                     dimension_row.column_type)); --if there were no rows inserted into a chunk, r_end wouldn't be defined.
    r_end = _timescaledb_internal.time_literal_sql(max_time_internal + 1,
                                                   dimension_row.column_type); -- add one here, so that we can still use < rather than <= (our internal representation is a bigint)
    EXECUTE FORMAT(unformatted_move_stmt
        , source
        , dimension_row.column_name
        , r_start
